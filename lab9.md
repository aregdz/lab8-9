...
**Физическая репликация - Механизм копирования всех изменений на уровне файлов СУБД (WAL-журналов) от мастера на реплики. Передаётся бинарный поток данных. Позволяет создать один или несколько ведомых серверов, полностью повторяющих содержимое основного.**

**Логическая репликация - Передаёт изменения на уровне отдельных таблиц (строк), а не всего сервера. Позволяет выбирать, какие объекты реплицировать, и использовать разные версии PostgreSQL между узлами. Основана на механизме публикаций и подписок.**
...
**max_standby_streaming_delay - Параметр, управляющий поведением реплики при конфликте изменения страниц и длительного запроса — можно либо прервать запрос, либо задержать применение изменений.**

**hot_standby_feedback - Механизм обратной связи от реплики, который запрещает мастеру удаление страниц, пока реплика их использует (защита от конфликтов).**

**Слот репликации - Объект, который удерживает WAL-сегменты до их фактической передачи на реплику. Используется, чтобы реплика не потеряла данные, даже если временно отключена.**

**VACUUM - Процедура очистки неиспользуемых версий строк и освобождения места в таблицах. Может конфликтовать с чтением на реплике.**

**Failover (переключение) - Процесс перевода реплики в роль мастера при отказе основного сервера. Позволяет продолжить работу без остановки системы.**

**pg_promote() - Команда, активирующая реплику и выводящая её из режима восстановления, превращая в новый основной сервер.**
